...
const ipData = require('../attached_assets/ips.json');
...
// Function to group IPs by ISP
function groupByISP(ips) {
  const isps = {
    'Liquid': [],
    'Telone': [],
    'Other': []
...
// Function to group IPs by type
function groupByType(ips) {
  const types = {
    'Internet': [],
    'Link': [],
    'VPN': [],
  ...
  ips.forEach(ip => {
    const { type } = parseIPName(ip.name);
    types[type].push(ip);
  });
...
// Import the storage from index.js
...
async function importRoutersAndIPs() {
  try {
    // We already have ipData from the require above
